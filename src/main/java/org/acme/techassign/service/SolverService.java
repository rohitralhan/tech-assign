...
import org.acme.techassign.solver.change.PinJobProblemChange;
...
public class SolverService {
    ...
    private void pinCallAssignedToAgents(List<Job> jobs) {
        jobs.stream()
                .filter(job -> !job.isPinned()
                        && job.getPreviousJobOrTech() != null
                        && job.getPreviousJobOrTech() instanceof Tech)
                .map(PinJobProblemChange::new) 
